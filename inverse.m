function [] = inverse(leftHand, rightHand, leftLeg, rightLeg, head)
close all;
clc;
...
disp('-----Left H25---------')
thetasL(5) = 0;
[T, left] = fLeftHandH25(thetasL);
r=JacobianInverse(T,@fLeftHandH25,thetasL'+randn(6,1)*0.001);%thetasRL'+2*pi+randn(6,1)*0.001);
...
% [T, r, v] = fRightHandH25(thetasR);
% r=JacobianInverse(T,@fRightLeg,thetasR'+randn(6,1)*0.001);%thetasRL'+2*pi+randn(6,1)*0.001);
% 
% disp('-----Left Leg---------')
% thetasLL(1) = 0;
% thetasLL(2) = pi/2;
...
% r=JacobianInverse(T,@fRightLeg,thetasRL'+randn(6,1)*0.001);%thetasRL'+2*pi+randn(6,1)*0.001);
