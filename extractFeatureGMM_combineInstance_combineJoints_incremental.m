...
function file = extractFeatureGMM_combineInstance_combineJoints_incremental(a1, a2, s1, s2, e1, e2, mode)
...
%                         pos = pos ./ repmat(mynorm',[1,3]);                        
            ...
            if ~isempty(oneSubject)
                ...
                [Priors, Mu, Sigma, expData] = GMM_incremental(Data, nbStates, numFrame);
                save('Data.mat', 'Data');
                disp('>>>>>> regress expData for action.');
                save(['data/expData_action',num2str(action), '.mat'], 'expData');
                [Y,I]=sort(Mu(1,:));
                B=Mu(:,I);
