...
function file = encodeData_rotateTrain_eachTest(a1, a2, s1, s2, e1, e2, mode, gap, featureMode)
    nbStates = 10;    
    numFrame = 200;
    ...
    for action = a1 : a2
        ...
        if numInstance ~= 0
            ...
            switch mode
                case '../dataset_GMM/train'
                    for i = 1 : numInstance
                        inVideos = videos(1:gap);
                        inData = align(inVideos, numFrame, 'dimension');
                        [feature, tmpM] = encodeGMM(inData,nbStates, numFrame, action, subject);
                        M(i) = tmpM;
                        row = [action, feature];
                        file = [file; row];
                    ...
                    M = M(1:numInstance);
                    save(['data/M_action_', int2str(action), '.mat'], 'M');
                    numAct = numAct + 1;
                    
                case '../dataset_GMM/test' 
                    count = 0;
                    for i = 1 : numInstance
                        inVideos = videos(i);
                        inData = align(inVideos, numFrame, 'dimension');
                        fprintf('Test action %d, instance %d\t', action, i);
                        result = test_calLoglik(inData, numFrame, numAct);
                        if result ~= action
                            count = count + 1;
                        end
                    end
